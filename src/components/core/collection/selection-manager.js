...
export const { Provider: SelectionProvider, Consumer: SelectionConsumer } = React.createContext({
  __onSetActiveIndex: () => {},
...
export default class SelectionManager extends React.Component {
  ...
  render() {
    return (
      <SelectionProvider value={this.state}>
        { this.props.children }
      </SelectionProvider>
    )
  }
...
export const withListKeyProvider = Component => props => {
  ...
  return (
    <SelectionManager vertical={vertical} horizontal={horizontal} onFocusChange={onFocusChange} typeAhead={typeAhead} wrap={wrap}>
      <Component {...restProps} />
    </SelectionManager>
  )
};
